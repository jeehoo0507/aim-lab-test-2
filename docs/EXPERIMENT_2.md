...
**9. 비용·재개.** 기본 jobs1, 최대 seed 두 개 병렬. 각 프로세스 CPU threads2/workers2. `benchmark`가 실제 batch32·accumulation4의 단독/2개 병렬 처리량, 대표 validation/probe/checkpoint I/O와 VRAM을 측정한다. 3개 시드의 단독 tail까지 포함한 전체 예상 시간이 10% 이상 줄고 메모리 여유가 있을 때만 jobs2를 권장한다. `--jobs auto`가 최신 벤치 결과를 적용하며, 실행 직전 메모리가 부족해졌으면 jobs1로 낮춘다. 완료 run 재사용, 미완료 run last 상태 재개. Config/데이터/teacher/core code fingerprint 불일치 시 혼합을 막고 새 output root를 요구한다. 완료 checkpoint는 약 3.8GB로 추정한다. 출력 권장 계획값 5GB, 경고 기준 10GB이며 데이터·환경/cache는 별도다. 초과 예상은 보고하되 용량 기준으로 학습을 강제 중단하지 않는다.
