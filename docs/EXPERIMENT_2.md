...
**9. 비용·재개.** 기본 jobs1, 명시적 jobs2로 seed 두 개까지만 병렬. 각 프로세스 CPU threads2/workers2. CUDA VRAM은 실제 batch32 benchmark로 확인하고 시간은 서버 pilot 실측으로 예측한다. 완료 run 재사용, 미완료 run last 상태 재개. Config/데이터/teacher/core code fingerprint 불일치 시 혼합을 막고 새 output root를 요구한다.
